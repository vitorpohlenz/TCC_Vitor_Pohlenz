...
for (t in tmax:2) {
  ...
  hidro = pot %>% filter(nome =="H")
  volback[t+1] = hidro[nrow(hidro),"liminf"] # valume para calcular custo apenas 
  ...
  cusback[t] = as.matrix(gback[t,1:3]) %*% termicas$ca + as.matrix(hidro[c("ca","cl")]) %*% c(volback[t+1],1)
}
