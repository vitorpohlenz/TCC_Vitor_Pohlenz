...
**OBS: ca = "coeficiente angular"; cl = "coeficiente linear"**

```{r termo}
knitr::kable(termicas)
```
...
$$
min f = \sum_{t=1}^{T} \sum_{k=1}^{3} (c_{t,k} \cdot p_{t,k} + \alpha_t)
$$
$$
s.t.: \sum_{k=1}^{3}(p_{t,k} )+ gh_t \geq L ~ ; ~t = 1,2,...,T
$$
$$
v_t = v_{t-1} + y_t - q_t - s_t ~ ; ~ t = 1,2,...,T
$$
$$
gh_t = \rho \cdot q_t ~ ; ~ 0 \leq p_{t,k} \leq P^{max}_{k} ~; ~ 0 \leq q_t \leq q^{max} ~; ~\alpha_t \geq 0
$$
